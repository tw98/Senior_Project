...
SUMMARYFILE = 'results/ind_mrAE_classification.csv'
def main():
    ...
    hidden = hidden.reshape(1, args.n_TR, -1)
    alhidden = alhidden.reshape(1, args.n_TR, -1)
    saveto = os.path.join(savepath, f"pt{args.pt}e{args.n_epochs}_symm{args.symm}_amlp.npy")
    np.save(saveto, alhidden)
    saveto = os.path.join(savepath, f"pt{args.pt}e{args.n_epochs}_symm{args.symm}_manihidden.npy")
